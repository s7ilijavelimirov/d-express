...
class D_Express_Order_Timeline
{
    ...
    public function register_timeline_assets($hook)
    {
        ...
        if (
            !$screen || !in_array($hook, $allowed_screens) ||
    ...
    private function render_single_shipment_timeline($shipment, $shipment_number, $total_shipments)
    {
        ...
        if ($package_count === 1 && !empty($packages)) {
            ...
            $package_info = sprintf(
                __('%d paketa (%s%s)', 'd-express-woo'),
                $package_count,
                implode(', ', $package_codes),
                $more
            );
        } else {
     ...
     */
    private function render_shipment_html(
        $shipment,
        $shipment_number,
        $is_multiple,
        $location_name,
    ...
    public function simulate_realistic_statuses($shipment_id)
    {
        ...
        foreach ($simulation_timeline as $sim) {
            if (
                ...
                if ($order) {
                    // Pozovi hook koji će email manager obraditi
                    do_action('dexpress_shipment_status_updated', $shipment, $sim['status'], dexpress_get_status_name($sim['status']), $order);
                }
            }
        }
